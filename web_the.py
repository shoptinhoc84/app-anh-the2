...
# Đoạn này giúp App không bị sập màn hình trắng nếu thư viện lỗi
try:
...
try:
    ...
    # Kiểm tra kỹ thuộc tính solutions để tránh AttributeError
    if hasattr(mp, 'solutions') and hasattr(mp.solutions, 'face_detection'):
        mp_face_detection = mp.solutions.face_detection
        HAS_MEDIAPIPE = True
    else:
        # Fallback nếu import được nhưng thiếu module con
        HAS_MEDIAPIPE = False
except ImportError:
    HAS_MEDIAPIPE = False
...
def resize_image_input(image, max_height=1000): # Giảm xuống 1000 để tránh tràn RAM
    w, h = image.size
    if h > max_height:
        ratio = max_height / h
        new_w = int(w * ratio)
...
def detect_face_mediapipe(image_cv, manual_angle=0):
    if not HAS_MEDIAPIPE:
        return None, 0, "Lỗi thư viện MediaPipe (Xem Requirements)"
...
def crop_final_image_v3(no_bg_img, manual_angle, target_ratio):
    try:
        if not HAS_MEDIAPIPE:
            # Fallback nếu không có AI: Cắt giữa ảnh đơn giản
...
if not HAS_MEDIAPIPE:
    st.error("⚠️ AI NHẬN DIỆN (MediaPipe) CHƯA ĐƯỢC KÍCH HOẠT. Vui lòng kiểm tra file requirements.txt. Đang chạy chế độ Manual (Thủ công).")
...
with st.sidebar:
    ...
    st.markdown("---")
    size_option = st.radio("Chọn cỡ ảnh:", ["5x5 cm (Visa Mỹ)", "3.5x4.5 cm (Visa Úc/Hàn/Âu)", "3.3x4.8 cm (Visa Trung Quốc)", "4x6 cm (Hộ chiếu)", "3x4 cm (Giấy tờ)"])
